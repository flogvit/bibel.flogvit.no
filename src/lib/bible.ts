...
export function getNumberingSystems(): { id: string; name: string; description: string | null }[] {
  const db = getDb();
  return db.prepare('SELECT id, name, description FROM numbering_systems ORDER BY is_default DESC, name').all() as { id: string; name: string; description: string | null }[];
}

export interface ChapterByNumberingResult {
  verses: Verse[];
  displayMap: Record<number, { chapter: number; verse: number }>;
  numberingSystem: string;
}

export function getChapterByNumbering(
  systemId: string, bookId: number, chapter: number, bible: string
): ChapterByNumberingResult | null {
  const db = getDb();

  // Get the KVN range for this chapter in the requested numbering system
  const range = db.prepare(
    'SELECT kvn_start, kvn_end FROM chapter_kvn_ranges WHERE system_id = ? AND book_id = ? AND chapter = ?'
  ).get(systemId, bookId, chapter) as { kvn_start: number; kvn_end: number } | undefined;

  if (!range) return null;

  // Get verses by KVN range
  const rows = db.prepare(
    'SELECT * FROM verses WHERE kvn BETWEEN ? AND ? AND bible = ? ORDER BY kvn'
  ).all(range.kvn_start, range.kvn_end, bible) as (Verse & { versions: string | null })[];

  const verses = rows.map(row => ({
    ...row,
    versions: row.versions ? JSON.parse(row.versions as string) : undefined
  }));

  // Get display numbers from the numbering system
  const displayRows = db.prepare(
    'SELECT kvn, chapter, verse FROM verse_kvn_map WHERE system_id = ? AND kvn BETWEEN ? AND ? ORDER BY kvn'
  ).all(systemId, range.kvn_start, range.kvn_end) as { kvn: number; chapter: number; verse: number }[];

  // Build map from verse.verse (internal) to display coordinates
  // We need to map by KVN: verse's kvn -> display chapter:verse
  const kvnToDisplay = new Map(displayRows.map(r => [r.kvn, { chapter: r.chapter, verse: r.verse }]));

  const displayMap: Record<number, { chapter: number; verse: number }> = {};
  for (const verse of rows) {
    const display = kvnToDisplay.get(verse.kvn);
    if (display) {
      displayMap[verse.id] = display;
    }
  }

  return { verses, displayMap, numberingSystem: systemId };
}

export function getChapterCountByNumbering(systemId: string, bookId: number): number {
  const db = getDb();
  const result = db.prepare(
    'SELECT MAX(chapter) as max_chapter FROM chapter_kvn_ranges WHERE system_id = ? AND book_id = ?'
  ).get(systemId, bookId) as { max_chapter: number } | undefined;
